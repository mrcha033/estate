...
def generate_weekly_report(self):
    try:
        ...
        # Placeholder for fetching subscribers from DB
        subscribers = [{"email": "test@example.com"}] # Replace with actual DB fetch
        report_url = f"http://your-app-domain.com/reports/{report_filename}" # Replace with actual report URL

        for subscriber in subscribers:
            send_report_email.delay(subscriber["email"], report_url, summary)
...
@shared_task
def send_report_email(recipient_email: str, report_url: str, report_summary: str):
    subject = "Your Weekly Real Estate Report is Here!"
    body = f"<h1>Weekly Real Estate Report</h1><p>Here's your weekly summary: {report_summary}</p><p>View full report: <a href=\"{report_url}\">{report_url}</a></p>"
    try:
